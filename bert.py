...
class BertSelfAttention(nn.Module):
	...
	def attention(self, key, query, value, attention_mask):
		...
		S = torch.matmul(query, key.transpose(2, 3)) / math.sqrt(self.attention_head_size)
...
class BertLayer(nn.Module):
	...
	def forward(self, hidden_states, attention_mask):
		...
		# add-norm layer
		normalized_mha_out = self.add_norm(hidden_states, mha_out, self.attention_dense, 
										self.attention_dropout, self.attention_layer_norm)
		# feed forward
		ff_out = self.interm_af(self.interm_dense(normalized_mha_out))
		# another add-norm layer
		normalized_ff_out = self.add_norm(normalized_mha_out, ff_out, self.out_dense,
